...
We can see a variety of patterns in time series. To understand and or model a time series, it is often helpful to split a time series into several components, each of which represents an underlying pattern category. These categories are:
...
The  kinds of models are tested here:
...
```
```{r}
cbind("Sales ($million)" = ts_companies_clean,
      "Monthly log sales" = log(ts_companies_clean),
      "Annual change in log sales" = diff(log(ts_companies_clean),12)) %>%
